...
import {
  ...
  getAttendance,
  editattendace,
} from "../Attendance/Reducer/AttendanceSlice";
...
const AttendanceList = ({ data, onTotalSalaryChange }) => {
  ...
  useEffect(() => {
    dispatch(getAttendance(userData._id, selectedMonth.value));
  }, [dispatch, userData._id, selectedMonth]);
  ...
  //     return recordMonth === selectedMonthName && recordYear === selectedYear;
  //   });
  // }, [data, selectedMonth]);
  const filteredData = useMemo(() => {
    ...
    return data.filter((record) => {
      ...
      return recordMonth === selectedMonthName && recordYear === selectedYear && dayjs(recordDate).isBefore(today);
    });
  }, [data, selectedMonth]);
  ...
  const fetchDataForSelectedMonth = (selectedMonthValue) => {
    ...
    dispatch(getAttendance(userData._id, selectedMonthValue));
  };
  ...
  const COLUMNS = [
    ...
    },
    {
      ...
      Cell: ({ cell }) => <span>{dayjs(cell.value).format("HH:mm:ss")}</span>,
    },
    {
  ...
  const actions = [
    ...
    },
    {
      doit: (item) => {
        ...
        setTimeout(() => {
          dispatch(getAttendance(userData._id));
        }, 500);
      },
    },
  ];
  ...
  useEffect(() => {
    onTotalSalaryChange(totalDailySalary); // Pass totalDailySalary to the parent component
  }, [totalDailySalary, onTotalSalaryChange]);
            ...
            <Controller
              ...
              render={({ field }) => (
                <Select
                  ...
                  onChange={(selected) => fetchDataForSelectedMonth(selected.value)}
                />
              )}
            />
          </div>
